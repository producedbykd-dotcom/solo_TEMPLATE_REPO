...
begin
  ...
  -- Active test drive: 1 release total, 1 analysis regen, 3 image regens.
  -- Stays entitled while the subscription is active so in-plan AI keeps
  -- working after the single upload is consumed.
  if sub.id is not null and sub.status = 'active' and sub.kind = 'test_drive' then
    return query select true,
      ...
      greatest(1 - sub.test_drive_analysis_regens_used, 0)
        + greatest(3 - sub.test_drive_image_regens_used, 0);
    return;
  end if;
